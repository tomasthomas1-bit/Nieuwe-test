...
from fastapi import (
    FastAPI, HTTPException, Depends, status, Response, Request, WebSocket, WebSocketDisconnect
)
...
FRONTEND_ORIGINS = os.environ.get("FRONTEND_ORIGINS", "")  # bv: "https://app.domein.com,https://staging.domein.com"
...
def init_pool() -> None:
    """Initialiseer één thread-safe connectionpool voor de app."""
    global pool
    if pool is None:
...
async def get_current_user(
    ...
    user = {
        "id": row[0], "username": row[1], "name": row[2], "age": row[3], "bio": row[4],
        "sport_type": row[5], "avg_distance": row[6], "last_lat": row[7], "last_lng": row[8],
        "availability": row[9], "preferred_sport_type": row[10], "preferred_min_age": row[11],
        "preferred_max_age": row[12], "strava_token": row[13]
    }
    return user
...
def on_startup():
    ...
    with DB() as (conn, c):
        ...
        c.execute("CREATE INDEX IF NOT EXISTS idx_chats_match ON chats (match_id) WHERE deleted_at IS NULL")
...
async def login_for_access_token(
    ...
    conn, c = db
    try:
        c.execute("SELECT password_hash FROM users WHERE username = %s AND deleted_at IS NULL",
                  (form_data.username,))
        row = c.fetchone()
...
async def update_user_preferences(
    ...
    try:
        ...
        """, (preferences.preferred_sport_type, preferences.preferred_min_age,
              preferences.preferred_max_age, user_id))
    ...
    for s in suggestions:
        ...
        if distance <= 250:
            suggestions_with_dist.append({
                "id": s[0], "name": s[1], "age": s[2], "bio": s[3], "sport_type": s[4],
                "avg_distance": s[5], "lat": s[6], "lng": s[7],
...
async def send_message(message: MessageIn, current_user: dict = Depends(get_current_user), db=Depends(get_db)):
    ...
    c.execute("""
        ...
        WHERE (swiper_id = %s AND swipee_id = %s AND liked = TRUE AND deleted_at IS NULL)
          AND EXISTS (
              SELECT 1 FROM swipes WHERE swiper_id = %s AND swipee_id = %s AND liked = TRUE AND deleted_at IS NULL
          )
...
async def get_chat_messages(match_id: int, current_user: dict = Depends(get_current_user), db=Depends(get_db)):
    ...
    c.execute("""
        SELECT 1 FROM swipes
        WHERE (swiper_id = %s AND swipee_id = %s AND liked = TRUE AND deleted_at IS NULL)
          AND EXISTS (SELECT 1 FROM swipes WHERE swiper_id = %s AND swipee_id = %s AND liked = TRUE AND deleted_at IS NULL)
    """, (user_id, match_id, match_id, user_id))
    if not c.fetchone():
